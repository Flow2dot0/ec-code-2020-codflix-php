...
class history_season_base {
  ...
  function nbRows() {
    global $pdo;
    $param = array();
    $query = "SELECT count(*) as NB FROM `history_season` WHERE 0=0";
  ...
  function getData($obj = true) {
    global $pdo;
    $param=array();
    $query  = "SELECT * FROM `history_season` WHERE 0=0";
    if(!empty($this->id) || (isset($this->id) && $this->id === 0)) {
      $query .= " AND `id`";
  ...
  function getRow($id) {
    global $pdo;
    $query = "SELECT * FROM `history_season` WHERE id= :id";
    try {
      $prepare = $pdo->prepare($query);
  ...
  function insertRow($nolog = 0) {
    ...
    $query = "INSERT INTO `history_season` SET";
    $query .= " `user_id` = :user_id";
    $param["user_id"] = !empty($this->user_id) || $this->user_id != "" ? $this->user_id : NULL;
    $query .= ", `season_id` = :season_id";
    $param["season_id"] = !empty($this->season_id) || $this->season_id != "" ? $this->season_id : NULL;
  ...
  function updateRow($nolog = 0) {
    ...
    $query = "UPDATE `history_season` SET";
    $query .= " `user_id` = :user_id";
    $param["user_id"] = !empty($this->user_id) || $this->user_id != "" ? $this->user_id : NULL;
    $query .= ", `season_id` = :season_id";
    $param["season_id"] = !empty($this->season_id) || $this->season_id != "" ? $this->season_id : NULL;
  ...
  function deleteRow() {
    ...
    if(!empty($this->id)) {
      ...
      $query = "DELETE FROM `history_season` WHERE id= :id";
      try {
        $prepare = $pdo->prepare($query);
